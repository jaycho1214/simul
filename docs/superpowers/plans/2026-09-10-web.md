...
- `@simul/protocol` is imported with `import type` only. The bundle therefore has zero workspace runtime coupling.
...
`@simul/protocol` is a **dev**Dependency on purpose: it is consumed with
...
- Consumes: `ServerMessage`, `TranscriptLine`, `LaneState` from `@simul/protocol` (type-only).
- Produces:
...
- Consumes: `ServerMessage` from `@simul/protocol` (type-only).
- Produces:
...
- Consumes: `TranscriptLine` from `@simul/protocol` (type-only).
...
- `pnpm --filter @simul/web build` produces `apps/web/dist`, and the Node server serves it at `/`.
