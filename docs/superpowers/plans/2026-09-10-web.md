...
body {
  ...
  color: var(--fg);
  font-family:
    -apple-system, BlinkMacSystemFont, "Apple SD Gothic Neo", "Noto Sans KR", "Segoe UI", Roboto,
    sans-serif;
  font-size: 18px;
  line-height: 1.4;
...
describe("parseConfig", () => {
  ...
  test("rejects an empty language list", () => {
    expect(() => parseConfig({ offeredLanguages: [], sourceLanguage: "ko" })).toThrow(
      /offeredLanguages/,
    );
  });
  ...
  return (
    <main className="picker">
      ...
      <ul className="picker-rows">
        {rows.map((row) => (
          <li key={row.lang}>
            <button type="button" className="picker-row" onClick={() => onPick(row.lang)}>
...
export function ListenScreen(props: ListenScreenProps) {
  ...
  return (
    <main className="listen">
      ...
      <StatusBanner banner={banner} onRetry={props.transcript.error ? props.onRetry : undefined} />
...
import { AudioStreamController, type AudioStatus } from "./audio/audio-stream-controller.ts";
