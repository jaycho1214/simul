...
- **Server hosting:** Electron `utilityProcess.fork()` — crash-isolated, restartable. A `--external-server` flag skips the fork during development.
- **The Opus encoder is `opusscript`, not `@discordjs/opus`.** The completed `apps/server` depends on `opusscript@^0.1.1`; `@discordjs/opus` was dropped because it segfaults on real audio. This removes an entire class of packaging work from this plan: `opusscript` is emscripten-compiled JavaScript, so there is **no native addon, no `rebuildConfig`, no ABI matching and no `.node` file to unpack**. It does load `build/opusscript_native_wasm.wasm` from disk with `fs.readFileSync(__dirname + …)`, so it must be marked **external** in the server bundle rather than inlined by Rollup — that is the only packaging constraint it imposes. See Task 7 Step 7 and Task 11.
- **`LaneOpusEncoder.close()` must run on teardown.** `opusscript` allocates its encoder on a module-level WASM heap that is freed only by an explicit `delete()`, which `LaneOpusEncoder.close()` calls. Lanes open and close on subscriber refcount for the whole event, so the operator app's shutdown path must go through `server.close()` (which reaches `LaneManager.closeAll()` → `lane.close()` → `encoder.close()`) and never through a bare `process.exit()`. Task 7 Step 6 is written that way deliberately.
...
- **The venue laptop is Windows. The development machine is macOS.** Anything that only exists on one of them — the Squirrel maker and its `tongyeok-setup.exe`, the PowerShell firewall probe, the XR18 WDM device names — **cannot be verified on macOS** and is marked as a pre-event check in Task 12 rather than assumed to work.
...
```
apps/operator/
  forge.config.ts               makers, asar unpack, Info.plist
...
Electron Forge and `@electron/packager` resolve the Electron binary and copy the
app's dependencies by walking a real `node_modules` tree. pnpm's default
symlinked layout breaks that, and it is also how `opusscript`'s
`build/opusscript_native_wasm.wasm` ends up inside the packaged app. Create
`/Users/jay/Codes/tongyeok/.npmrc`:
...
fails loudly if someone adds an English label, and documents the thirteen places
where an English token is intentional. `level.rms` (RMS) and `reach.macos`
(macOS, Windows) are excluded outright rather than pinned, because both are
entirely English by nature.
...
Run: `cd apps/operator && pnpm test`
Expected: PASS, 30 tests — 5 i18n + 7 pcm + 9 frame-accumulator + 9 level (3 in the
`toDb` block and 6 in the `measureLevel` block). Confirm the reported total is 30
and that no test is skipped.
...
Run: `cd apps/operator && pnpm test`
Expected: PASS, 55 tests total. The `reconnecting-socket` and `ingest-socket`
...
Run: `cd apps/operator && pnpm test && pnpm typecheck`
Expected: PASS, 60 tests total. Typecheck clean — note that `?url` imports need
...
export interface ServerSupervisorOptions {
   ...
   * the engineer can paste a new Gemini API key into 제어 mid-event, and a
   * subsequent 서버 재시작 has to fork with the new key rather than the one that
...
async function main(): Promise<void> {
  ...
  process.parentPort.on("message", (event) => {
    ...
    if (message?.type === "shutdown") {
      ...
      // lane for the life of the process.
...
export const supervisor = new ServerSupervisor({
  ...
  // function so every spawn picks up the settings as they are now.
  env: () => ({ PORT: "8080" }),
...
void app.whenReady().then(() => {
  // Fork before the window opens. The renderer's /ingest and /admin sockets
  // both reconnect on their own, so a server that is still binding when the
  // window appears is not a problem — but a server nobody ever started is.
  supervisor.start();
});
...
app.on("will-quit", (event) => {
  ...
  // turns into server.close(), which closes every lane's LaneOpusEncoder.
  void supervisor.stop().then(() => app.exit(0));
});
```

`app.whenReady()` already has a handler from Task 5 Step 5 (the media permission
handler). Put `supervisor.start()` in that same handler rather than registering a
second one.
...
`apps/operator/src/ipc/router.ts` — extend the template's router. Task 1 Step 2
kept `src/ipc/theme/**` and `src/ipc/window/**` (the custom title bar needs the
window procedures) and deleted everything else, so the whole file is:
```ts
...
import * as window from "./window/index.ts";
...
If the template's own `router.ts` exports its namespaces under different names,
keep its names for `theme` and `window` and only add the three new ones — the
renderer's `ipc.client.theme.*` / `ipc.client.window.*` call sites come from the
template and must keep resolving.

- [ ] **Step 6: Complete the main process wiring**

In `apps/operator/src/main.ts`, replace the `env` stub from Task 7 Step 8 with the
real read:
```ts
...
The environment is only re-read at spawn time, so writing a new API key does not
take effect until the server restarts. That restart is deliberately **explicit**:
`settings.set` returns the updated settings and the 제어 panel (Task 10) offers
서버 재시작. An implicit restart mid-event would cut every listener off without
warning.
      ...
      <dl className="mt-4 space-y-1 text-sm">
        ...
        <div>
          {t("device.sampleRate", {
            ...
            context: report?.contextSampleRate ?? 0,
          })}
        </div>
...
laptop). Two things make that awkward and each is handled explicitly: the server
bundle keeps `opusscript` external and that package reads a `.wasm` data file off
disk relative to `__dirname`, and the Windows installer cannot be produced from
macOS.

**What this task is *not*.** There is no native addon anywhere in this app.
`apps/server` encodes with `opusscript`, which is emscripten-compiled
JavaScript, so there is no `rebuildConfig`, no `@electron/rebuild` step, no
Node-ABI matching and no `.node` file. Anyone porting instructions from an
`@discordjs/opus` setup will add work that does nothing and a rebuild step that
fails.
...
`apps/operator/forge.config.ts` — replace `packagerConfig` and `makers`, delete
the template's `rebuildConfig` (nothing in this app is native), and keep the
`VitePlugin` block from Task 7 Step 7 and the `FusesPlugin` block from the
...
If the utility process dies immediately with `Cannot find module 'opusscript'`,
the package was not copied into the app — check that Task 1 Step 3's
`node-linker=hoisted` is in effect. If it instead dies with `ENOENT …
opusscript_native_wasm.wasm`, the unpack glob did not match. Confirm both:
```bash
APP=out/tongyeok-darwin-*/tongyeok.app/Contents/Resources
ls $APP/app.asar.unpacked/node_modules/opusscript/build/opusscript_native_wasm.wasm
npx @electron/asar list $APP/app.asar | grep opusscript | head
```
...
@tongyeok/operator make`. Do not ship a `--platform=win32` cross-build from
macOS: Squirrel's installer stub is assembled with Windows tooling, and an
installer that has never been run on Windows is not an artefact anyone should
find out about at a venue.
...
- [ ] **U1 — the five panels are present and Korean.** Start the app. Confirm
  ...
  the buttons read 시작 and 중지. No English appears except the thirteen pinned
  keys from Task 1's i18n test, plus RMS and the macOS/Windows names.
...
- [ ] **U5 — the processing sample rate is 16 kHz.** Confirm the 샘플레이트 line
  reads 처리 16000 Hz. The 장치 figure will normally read 44100 or 48000 — that is
  the interface's own rate and is fine. If 처리 shows anything but 16000, the
  `AudioContext({ sampleRate: 16000 })` was refused, the amber
  `sample_rate_mismatch` warning should also be visible, and every frame reaching
  the server is at the wrong rate.
...
- [ ] **S6 — an *invalid* API key fails loudly too.** This is the spec's other
  failure row — "API key invalid or quota exhausted → fail loudly on the operator
  dashboard, not silently per lane". `loadConfig` only checks that a key is
  present, so a wrong key starts the server cleanly and fails on the first
  translated lane. Save a deliberately wrong key, restart, then open
  `/listen?lang=en` from a phone. Confirm 레인 현황 shows the English row with
  세션 상태 오류 and the amber degraded highlight, and that it does **not** sit on
  시작 중 forever. If the row stays 시작 중, the operator app cannot see the
  failure and `apps/server`'s lane error propagation needs fixing — record it
  rather than working around it here.

- [ ] **S7 — a graceful stop frees the encoders.** Quit the app from the window
  rather than killing it. The `will-quit` handler must reach
  `supervisor.stop()` → `{ type: "shutdown" }` → `server.close()`, which is what
  calls `LaneOpusEncoder.close()` and frees `opusscript`'s WASM heap. Confirm the
  utility process exits on its own within the supervisor's 3 s force-kill window
  rather than being killed — a forced kill means lanes never closed.
...
- [ ] **P5 — `opusscript` loaded and its WASM is on disk.** The server reached
  `listening` rather than crashing on `Cannot find module 'opusscript'` or
  `ENOENT … opusscript_native_wasm.wasm`. Confirm the unpacked file exists under
  the installed app's `resources\app.asar.unpacked\node_modules\opusscript\build\`.
  There is no ABI to check — `opusscript` is JavaScript, so a package that loads
  on macOS loads identically here.
...
- Development checklist items U1–U9, S1–S7 and P1–P3 all pass on macOS.
...
4. **An invalid Gemini key is only visible once a lane opens.** The spec asks for
   "fail loudly on the operator dashboard, not silently per lane". A *missing*
   key does fail loudly — `loadConfig` throws and the supervisor shows the
   crashed state with the message (checklist S5). An *invalid* key passes
   `loadConfig`, so it only surfaces when the first translated lane goes to
   세션 상태 오류 in 레인 현황 (checklist S6). Making that louder — a validation
   call at startup, or a dashboard-level banner rather than a per-row state —
   requires a change to `apps/server`, so it is recorded rather than bodged into
   the operator app.
5. **XR18 channels 9–18 remain unreachable on Windows.** No amount of UI fixes
