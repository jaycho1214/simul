...
- **`packages/protocol` exports types only.** Every import from it must be `import type { … } from "@simul/protocol"`. `INGEST_FRAME_BYTES` is _not_ there — the server keeps its own copy in `apps/server/src/constants.ts` and the operator app keeps its own in `apps/operator/src/capture/frame-accumulator.ts`.
...
- **The venue laptop is Windows. The development machine is macOS.** Anything that only exists on one of them — the Squirrel maker and its `Simul-Setup.exe`, the PowerShell firewall probe, the XR18 WDM device names — **cannot be verified on macOS** and is marked as a pre-event check in Task 12 rather than assumed to work.
...
- Consumes: nothing.
- Produces: a workspace package `@simul/operator` that starts with `pnpm --filter @simul/operator start`, a Korean-only i18next instance exporting `t` and `KO_STRINGS`, and a `vitest run` script that the root `pnpm -r test` picks up.

- [ ] **Step 1: Land the template**

```bash
cd /Users/jay/Codes/simul
...
```bash
cd /Users/jay/Codes/simul/apps/operator
...
```bash
cd /Users/jay/Codes/simul
pnpm --filter @simul/operator add @simul/protocol@workspace:* @simul/server@workspace:* electron-store qrcode
pnpm --filter @simul/operator add -D @types/qrcode ws @types/ws
pnpm install
```
...
- Consumes: `createServer`, `loadConfig`, `SystemClock`, `createGeminiTranslateSessionFactory` from `@simul/server`.
- Produces:
...
export class ServerSupervisor {
  ...
  start(): void {
    if (this.opts.external) {
      // The engineer is running `pnpm --filter @simul/server dev` in another
...
(`simul server on :8080`, `languages: ko, en, es, ja (source ko)`) appear,
...
(`Activity Monitor` → `simul-server`, or `pkill -f simul-server`).
...
```bash
pnpm --filter @simul/server dev &
pnpm --filter @simul/operator start:external
```

Expected: no `simul-server` utility process is spawned and the 제어 panel reads
외부 서버 사용 중.
...
- Consumes: `type LaneStatus`, `type AdminMessage`, `type LaneState` from `@simul/protocol` (**type-only imports**); `ReconnectingSocket` from Task 4.
- Produces:
...
cd /Users/jay/Codes/simul/apps/operator
pnpm package
open out/Simul-darwin-*/Simul.app
```
...
```bash
APP=out/Simul-darwin-*/Simul.app/Contents/Resources
...
| ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `MakerSquirrel` produces a working `Simul-Setup.exe`              | Squirrel packaging requires Windows tooling; a cross-built installer is not a tested artefact |
...
@simul/operator make`. Do not ship a `--platform=win32` cross-build from
...
- [ ] **U2 — device labels appear only after permission.** With a fresh
      `~/Library/Application Support/Simul/`, confirm the device dropdown
...
- [ ] **S1 — the server runs in its own process.** With the app running,
      `ps -A | grep simul-server` shows a separate process.

- [ ] **S2 — a server crash does not take the window down.** `pkill -f
simul-server`. The window stays up, 제어 shows 서버가 비정상 종료되었습니다 with a
...
- [ ] **S4 — `--external-server` forks nothing.** Run
      `pnpm --filter @simul/server dev` in one terminal and
      `pnpm --filter @simul/operator start:external` in another. Confirm no
      `simul-server` utility process exists and 제어 reads 외부 서버 사용 중.
...
- [ ] **P4 — the Windows build exists and was built on Windows.**
      `pnpm --filter @simul/operator make` was run on this machine or a Windows
      runner, and `out/make/squirrel.windows/x64/Simul-Setup.exe` installs and
      launches.
...
- `pnpm --filter @simul/operator test` passes with 139 tests.
- `pnpm -r typecheck` is clean.
- `pnpm --filter @simul/operator package` produces a macOS bundle that starts,
