...
- **`packages/protocol` exports types only.** Every import from it must be `import type { … } from "@tongyeok/protocol"`. `INGEST_FRAME_BYTES` is _not_ there — the server keeps its own copy in `apps/server/src/constants.ts` and the operator app keeps its own in `apps/operator/src/capture/frame-accumulator.ts`.
...
The spec's failure table says: _"Operator app loses server → auto-reconnect;
capture keeps running so the level meter still moves; dashboard shows
disconnected."_ That means the socket must never throw into the audio path and
...
only prove that the shim was called. What _is_ tested is the offline chain the
...
The second is the spec's rough edge: _"Windows Firewall will silently block the
attendee port… Phones then fail to load the page with no error on the laptop. The
operator app should surface reachability rather than assume it."_
...
function profileMatches(rule: string, category: string): boolean {
  ...
  const wanted =
    category.toLowerCase() === "domainauthenticated" ? "domain" : category.toLowerCase();
...
The spec: _"the server runs in an Electron `utilityProcess.fork()` — crash-isolated
...
export function JoinInfoPanel({ externalListenerSeen }: { externalListenerSeen: boolean }) {
  ...
  const qr = useQuery({
    queryKey: ["qr", url],
    enabled: Boolean(url),
    queryFn: () => QRCode.toDataURL(url!, { width: 512, margin: 1, errorCorrectionLevel: "M" }),
  });
  ...
  return (
    <section className="rounded-lg border p-4">
      ...
      {url ? (
        <>
          {qr.data ? <img src={qr.data} alt={url} className="mx-auto h-64 w-64" /> : null}
...
export function ControlPanel() {
  ...
  return (
    <section className="rounded-lg border p-4">
      ...
      <p
        className={
          settings.data?.hasGeminiApiKey ? "text-sm text-emerald-700" : "text-sm text-amber-700"
        }
      >
...
**What this task is _not_.** There is no native addon anywhere in this app.
...
The spec's architecture diagram says the server _"Serves the attendee SPA
statically"_, but the completed server (`apps/server`, Task 16) exposes only
...
- [ ] **U3 — the DSP flags really report off.** Press 시작 against any device.
      ...
      _This is the check the spec exists for: an AGC that silently comes back on is
      invisible until it has already ruined an event._
...
- [ ] **S6 — an _invalid_ API key fails loudly too.** This is the spec's other
...
4. **An invalid Gemini key is only visible once a lane opens.** The spec asks for
   "fail loudly on the operator dashboard, not silently per lane". A _missing_
   key does fail loudly — `loadConfig` throws and the supervisor shows the
   crashed state with the message (checklist S5). An _invalid_ key passes
