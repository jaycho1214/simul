...
- Decision was *"build `SessionRotator`"* → implement exactly as written below.
- Decision was *"drop `SessionRotator`"* (naive reconnect seams under 1 s) →
...
export class SessionRotator implements TranslateSession {
  ...
  async rotate(): Promise<void> {
    ...
    const handle = (this.current as { resumptionHandle?: string } | undefined)
      ?.resumptionHandle;
...
export class SourceLane implements Lane {
  ...
  constructor(readonly lang: LangCode, opusBitrate: number) {
