...
- Produces: all wire types in `@tongyeok/protocol`, and `loadConfig(env): Config` returning the frozen `Config` object every later task depends on.
...
```ts
import { INGEST_FRAME_BYTES } from "@tongyeok/protocol";
import type { AudioHub } from "../audio-hub.ts";
