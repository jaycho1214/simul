...
ws.on("error", (e) => { console.error("could not connect:", e.message); process.exit(1); });
...
await new Promise((done) => {
  const t = setInterval(() => {
    ...
    for (let s = 0; s < 320; s++) {
      b.writeInt16LE(Math.round(9000 * Math.sin(2 * Math.PI * 440 * ((n * 320 + s) / 16000))), s * 2);
    }
    ws.send(b);
    if (++n >= 250) { clearInterval(t); ws.close(); done(); }
  }, 20);
});
console.log("done");
