...
test("drops clusters for a backed-up client instead of buffering", async () => {
  ...
  res.writableLength = 400_000; // past the 256 KB threshold
  ...
  assert.ok(route.listenerDrops(PASSTHROUGH_LANG) > 0);
});
...
test("releases the lane and ends the response when write() fails after headers are sent", async (t) => {
  ...
  const status = manager.statuses().find((s) => s.lang === PASSTHROUGH_LANG);
...
test("a client that disconnects while the lane is still opening does not leak the subscription", async () => {
  ...
  // a Gemini session) and genuinely takes time to settle — unlike the passthrough
  // lane, which resolves synchronously with no session at all.
...
test("a joining client receives recent clusters before any live one", async () => {
  const { route, hub } = setup();
  // Someone is already listening, so the lane is open and accumulating.
  const first = new FakeResponse();
  await route.handle(first as any, PASSTHROUGH_LANG);

  // 30 frames x 20 ms = 600 ms, several clusters' worth.
  for (let i = 0; i < 30; i++) hub.push(Buffer.alloc(640));

  const late = new FakeResponse();
  await route.handle(late as any, PASSTHROUGH_LANG);

  assert.ok(late.written[0]!.includes(Buffer.from("OpusHead")), "init segment first");
  assert.ok(
    late.written[1]?.includes(CLUSTER_ID),
    "then recent clusters, so the media element's duration gate clears at once",
  );
});
